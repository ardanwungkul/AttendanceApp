                ...
                @foreach ($divisi as $item)
                    <tr>
                        ...
                        <td class="flex justify-center items-end gap-3 text-xs">
                            ...
                            <button data-modal-target="{{ 'divisi-modal' . $item->id }}"
                                data-modal-toggle="{{ 'divisi-modal' . $item->id }}"
                                class="bg-red-500 px-3 py-1 rounded-lg text-white flex gap-2 items-center hover:bg-opacity-90">
                                <i class="fa-solid fa-trash"></i>
                                <span> Delete
                                </span>
                            </button>
                            <div id="{{ 'divisi-modal' . $item->id }}" tabindex="-1" aria-hidden="true"
                                ...
                                <div class="relative p-4 w-full max-w-2xl max-h-full">
                                    <form action="{{ route('divisi.destroy', $item->id) }}" method="POST"
                                        ...
                                        <p class="text-base text-gray-600 mb-3">
                                            Apakah kamu yakin untuk menghapus {{ $item->nama_divisi }} ?
                                        </p>
                                        <div class="w-full flex items-center justify-end gap-x-3">
                                            ...
                                            <div data-modal-hide="{{ 'divisi-modal' . $item->id }}"
